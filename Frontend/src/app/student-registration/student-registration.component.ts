...
export class StudentRegistrationComponent implements OnInit {
  ...
  selectedMarksheets: (File | null)[] = [null];
  ...
  get academicDetailsArray(): FormArray {
    return this.registrationForm.get('academicDetails') as FormArray;
  }
  ...
  private initializeForm(): void {
    this.registrationForm = this.fb.group({
      ...
      academicDetails: this.fb.array([
        this.createAcademicFormGroup()
      ])
    });
  }
  ...
  // Create a new academic form group
  ...
  addAcademicEntry(): void {
    this.academicDetailsArray.push(this.createAcademicFormGroup());
    this.selectedMarksheets.push(null);
  }

  // Remove an academic entry
  removeAcademicEntry(index: number): void {
    this.academicDetailsArray.removeAt(index);
  ...
  onFileSelected(event: Event, fileType: string): void {
    ...
    // Check if this is a marksheet upload
    if (fileType.startsWith('marksheet-')) {
      const index = parseInt(fileType.split('-')[1]);
      this.selectedMarksheets[index] = file;
      
      const academicGroup = this.academicDetailsArray.at(index) as FormGroup;
      academicGroup.patchValue({ marksheet: file });
      return;
    }
  ...
  private prepareFormData(): FormData {
    ...
    formValue.academicDetails.forEach((academic: any, index: number) => {
      Object.keys(academic).forEach(key => {
        if (key !== 'marksheet') {
          formData.append(`academicDetails[${index}].${key}`, academic[key]);
        }
      });
      
      // Add marksheet if available
      if (this.selectedMarksheets[index]) {
        formData.append(`marksheet-${index}`, this.selectedMarksheets[index] as File);
      }
    });
  ...
  formReset(): void {
    ...
    // Reset arrays
    while (this.academicDetailsArray.length > 0) {
      this.academicDetailsArray.removeAt(0);
    }
    this.addAcademicEntry();
    
    // Reset file properties
    this.selectedPhoto = null;
    this.selectedSignature = null;
    this.selectedResume = null;
    this.selectedMarksheets = [null];
  ...
  getAcademicControlError(index: number, controlName: string): string {
    const control = (this.academicDetailsArray.at(index) as FormGroup).get(controlName);
